...
2. Read `reviews/<review-name>/reading-queue.yaml` to find the highest-priority pending paper.

3. If no papers in queue, inform the user and suggest running `/search`.

4. Get the paper details from `references.yaml`.
...
6. Create a summary note at `reviews/<review-name>/notes/paper-summaries/<key>.md` using this template:
...
7. Update `references.yaml`: set `read: true` and `notes_file` path

8. Update `reading-queue.yaml`: set status to `done`

9. Add any interesting citations to the reading queue

10. Update relevant subtopics in `subtopics.yaml` with findings
